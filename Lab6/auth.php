...
function require_auth(): array
{
    $user = current_user();
    if ($user === null) {
        log_action('access_denied_unauthenticated', [
            'target' => basename($_SERVER['PHP_SELF'] ?? 'unknown'),
        ]);
...
function require_role(string $role): array
{
    $user = require_auth();
    if ($user['role'] !== $role) {
        log_action('access_denied_wrong_role', [
            'required_role' => $role,
...
function require_permission(string $permissionCode): array
{
    ...
    if (!has_permission($permissionCode)) {
        log_action('access_denied_permission', [
